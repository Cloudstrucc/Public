    ...
    if (Test-Path -Path $jsonFilePath -PathType Leaf) {
        ...
        Write-Host "{
            ...
            `"lookup`": {
                ...
                `"update`": {
                    `"newTargetEntities`": [`"opportunity`", `"lead`"]
                }
            }
        }"
        exit
...
if ($jsonConfig.lookup.operation -eq "create" -and (-not $jsonConfig.lookup.targetEntities -or $jsonConfig.lookup.targetEntities.Count -eq 0)) {
    Write-Host "Error: targetEntities array is required for create operation and must not be empty."
    exit
}

if ($jsonConfig.lookup.operation -eq "update" -and (-not $jsonConfig.lookup.update -or -not $jsonConfig.lookup.update.newTargetEntities -or $jsonConfig.lookup.update.newTargetEntities.Count -eq 0)) {
    Write-Host "Error: newTargetEntities array is required for update operation and must not be empty."
    exit
}
...
# Function to add new relationship to existing polymorphic lookup
function Add-PolymorphicRelationship {
    param (
        [PSCustomObject]$config,
        [string]$newEntity
    )
    
    $createUrl = $apiUrl + "RelationshipDefinitions"
    
    $relationshipRequest = @{
        SchemaName = "$($config.lookup.name.ToLower())_$newEntity"
    ...
    $createBody = $relationshipRequest | ConvertTo-Json -Depth 10

    try {
        Write-Host "Adding new relationship for entity $newEntity with payload:"
        Write-Host $createBody

        $response = Invoke-RestMethod -Uri $createUrl -Method Post -Body $createBody -Headers $headers -ContentType "application/json"
        Write-Host "Successfully added relationship for entity: $newEntity"
        return $response
    } catch {
        Write-Host "Error adding relationship for entity $newEntity : $_"
        $errorResponse = $_.ErrorDetails.Message
        if ($errorResponse) {
            Write-Host "Error details: $errorResponse"
        }
...
elseif ($jsonConfig.lookup.operation -eq "update") {
    Write-Host "Adding new relationships to existing polymorphic lookup..."
    
    $successCount = 0
    foreach ($newEntity in $jsonConfig.lookup.update.newTargetEntities) {
        Write-Host "Processing new relationship for entity: $newEntity"
        
        # Check if relationship already exists
        $existingRelationship = Get-ExistingRelationship -relationshipName "$($jsonConfig.lookup.name.ToLower())_$newEntity"
        if ($existingRelationship) {
            Write-Host "Relationship for entity $newEntity already exists. Skipping..."
            continue
        }

        $response = Add-PolymorphicRelationship -config $jsonConfig -newEntity $newEntity
        ...
        if ($response) {
            ...
            # Add to solution if specified
            if ($jsonConfig.solution -and $jsonConfig.solution.uniqueName) {
                Write-Host "Adding new relationship to solution..."
                Start-Sleep -Seconds 2  # Wait for relationship to be fully created
                
                $relationship = Get-ExistingRelationship -relationshipName "$($jsonConfig.lookup.name.ToLower())_$newEntity"
                if ($relationship -and $relationship.MetadataId) {
                    $addResult = Add-ComponentToSolution -solutionId $jsonConfig.solution.uniqueName `
                                                       -componentId $relationship.MetadataId `
                                                       -componentType 10
                    if ($addResult) {
                        Write-Host "Relationship for $newEntity added to solution successfully"
                    }
                } else {
                    Write-Host "Could not find newly created relationship for $newEntity"
                }
            }
        }
    }
    
    Write-Host "Update completed. Successfully added $successCount new relationships."
}
else {
    Write-Host "Invalid operation specified in configuration. Use 'create' or 'update'."
}
