...
The image below depicts the scope of technologies in M365 and Azure that support the platform. All of these have been assessed and implemented to support for non production and production implementations and are all "SAAS" or "PAAS" based thus are licensed based which provides transparency on costs due to subscription model rather than an architecture reliant on IAAS which is capacity based (thus harder to predict). This gives us the necessary oversight to cloud costs and now that this service is fully integrated in production in a PBMM context, many of our clients can benefit from reuse and significantly expdedidate their project by focusing on the business functionality rather than implicit core functioality to meet our security, accessbility, availability and data integrity requirements. The more systems are implemented using this model the more visitiliby and guardrails we have in creating secure applications which otherwise would need to be created from scratch. Therefore implementing the right support model and tooling for any internal service line to be able to spawn the technical artifacts from this framework will be key for platform adoption which in the short to long term will result in lower project failure risks, corporate knowledge being more decentralized, a standard on boarding model for staff to support this model, and finally resulting in a mature and efficient service line that is organically forced to optomize to industry standards and our own security standards. Moreover this makes it simpler for the organization management structure as it leads to a consolidation of effort into a framework that is a SAAS (like Teams).
