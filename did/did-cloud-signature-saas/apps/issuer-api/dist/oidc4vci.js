...
exports.registerOidc4vciRoutes = registerOidc4vciRoutes;
 ...
 */
function resolveOrigin(envOrigin, req) {
    if (envOrigin)
...
function registerOidc4vciRoutes(app, agent, explicitOrigin) {
     ...
     */
    app.post('/oidc/admin/create-offer', async (req, res) => {
        try {
            const origin = resolveOrigin(explicitOrigin, req);
            ...
            const qr_png_data_url = await qrcode_1.default.toDataURL(deepLink);
            res.json({
     ...
     */
    app.get('/oidc/credential-offer/:code', (req, res) => {
        const origin = resolveOrigin(explicitOrigin, req);
        const code = req.params.code;
        const offer = offers.get(code);
        if (!offer)
            return res.status(404).json({ error: 'unknown_offer' });
        const body = {
            credential_issuer: origin,
     ...
     *   tx_code=... (if required)
     */
    app.post('/oidc/token', express_1.default.urlencoded({ extended: true }), (req, res) => {
     ...
     */
    app.post('/oidc/credential', async (req, res) => {
        try {
            ...
            const jwt = typeof (vc?.verifiableCredential) === 'string'
